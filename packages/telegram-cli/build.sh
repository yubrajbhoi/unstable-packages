...
termux_step_post_configure() {
	sed -i -e 's@-I/usr/local/include@@g' \
		-e 's@-I/usr/include@@g' Makefile
}

termux_step_make_install() {
	install -Dm700 bin/telegram-cli \
		$TERMUX_PREFIX/bin/telegram-cli
	install -Dm600 tg-server.pub \
		$TERMUX_PREFIX/etc/telegram-cli/server.pub
	install -Dm600 debian/telegram-cli.8 \
		$TERMUX_PREFIX/share/man/man8/telegram-cli.8
}
